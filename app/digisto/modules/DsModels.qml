...
Item {
    // World currecy list model
    // { cc: '', symbol: '', name: ''}
    readonly property var worldCurrencyModel: [
    ...
    // Country Mobile list Model
    readonly property var worldMobileModel: [
        {
            name: "Afghanistan",
            dial_code: "+93",
            code: "AF"
    ...
    function findMobileFromModel(value, key='', compareFunc=null) {
        return findInModel(worldMobileModel, value, key, compareFunc)
    }

    function findCurrencyFromModel(value, key='', compareFunc=null) {
        return findInModel(worldCurrencyModel, value, key, compareFunc)
    }

    // Ensure the key and value can infer to `toLowerCase` function
    function findInModel(model, value, key, compareFunc=null) {
        try {
            for(var i=0; i<model.length; i++) {
                var model_i = model[i]
                if(compareFunc) {
                    if(compareFunc(model_i[key], value)) {
                        return model_i
                    }
                }

                else {
                    if(model_i[key].toLowerCase() === value.toLowerCase()) {
                        return model_i
                    }
                }
            }
